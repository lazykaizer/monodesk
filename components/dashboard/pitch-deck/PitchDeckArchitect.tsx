...
import CubeLoader from '@/components/ui/feedback/cube-loader';
...
export default function PitchDeckArchitect() {
    ...
    const saveToDatabase = async (currentSlides: Slide[], silent = false) => {
        ...
        try {
            ...
            // 1. ATOMIC SAVE: Save everything directly to the Database (JSONB column)
            console.log("Atomic Save: Writing all slide data to Database...");
            if (!silent) setSavingProgress("Synchronizing with Database...");

            const finalPayload = {
                id: deckId || undefined,
                deck_title: String(currentSlides[0]?.title || startupName || deckConfig.idea.substring(0, 30)).trim(),
                slides_content: currentSlides, // Everything saved directly in JSON
                idea: String(deckConfig.idea).trim(),
            };

            const data = await vaultPitchDeck(finalPayload);
            if (data && data.id) {
                if (!currentDeckIdRef.current) {
